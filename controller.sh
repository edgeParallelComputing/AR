...
#grid
serverSet=(10.10.21.1 10.10.21.2 10.10.21.4 10.10.21.5 10.10.21.6 10.10.21.7)
...
function bandwidthControl(){
  for((i=0; i<$serverCount; i++))
	...
	tc class add dev eth1 parent 1: classid 1:1 htb rate 130mbps
	tc class add dev eth1 parent 1:1 classid 1:11 htb rate 130mbps
+
    done
}
...
function cpuUtilization(){
    echo set cpu utilization $2 on $1    
    utilization=$2
...
function _utilization(){
        ...
        #$1 utilization, $2 node
        cpuUtilization $2 $1

        echo run the utlization test for $1

        echo submit topology
	submitStandAloneTopology $2 & >/dev/null 2>&1

        sleep 30

        echo collect log
	#collectSingleAloneLog $2
        #mv ./utilization/$2/reconstruction.log ./utilization/$2/allLog_$1.log
	mv /root/reconstruction.log ./utilization/$2/allLog_$1.log

	#kill topology
	echo kill topology
	killStandAloneTopology $2
...
function executeExperiment(){
	...
	mkdir /root/stormLogs/$fps/parallel
        for (( parallel=1128; parallel <=128; parallel=((parallel*2)))); do
