...
int PreFilterType = 0, PreFilterCap = 50, MinDisparity = 7, UniqnessRatio = 1,
    TextureThreshold = 0, SpeckleRange = 0, SADWindowSize = 68, smallBlockSize =
        10, SpackleWindowSize = 0, numDisparities = 159, PreFilterSize = 23,
    lambda(8000), sigma(1.5), vis(5), disp12MaxDiff(10);
...
void onTrackBar(int) {
  ...
  printf(
      ...
      MinDisparity - 30, TextureThreshold, SpackleWindowSize, UniqnessRatio,
      disp12MaxDiff);
...
void testDisparityParameters() {
  ...
  namedWindow("disparityTune", CV_WINDOW_AUTOSIZE);
  cvCreateTrackbar("DisparitiesNum", "disparityTune", &numDisparities, 255,
                   onTrackBar);
  cvCreateTrackbar("Block Size", "disparityTune", &SADWindowSize, 25,
                   onTrackBar);
  cvCreateTrackbar("Pre Filter Size", "disparityTune", &PreFilterSize, 255,
                   onTrackBar);
  cvCreateTrackbar("Pre Filter Cap", "disparityTune", &PreFilterCap, 61,
                   onTrackBar);
  cvCreateTrackbar("Minimum Disparity", "disparityTune", &MinDisparity, 10,
                   onTrackBar);
  cvCreateTrackbar("Uniqueness Ratio", "disparityTune", &UniqnessRatio, 30,
                   onTrackBar);
  cvCreateTrackbar("Speckle Window Size", "disparityTune", &SpackleWindowSize,
                   200, onTrackBar);
  cvCreateTrackbar("Filter Lambda", "disparityTune", &lambda, 20000,
                   onTrackBar);
  cvCreateTrackbar("Display", "disparityTune", &disp12MaxDiff, 100, onTrackBar);
  cvCreateTrackbar("Filter Sigma", "disparityTune", &sigma, 100, onTrackBar);
  cvCreateTrackbar("vis", "disparityTune", &vis, 10, onTrackBar);

  setMouseCallback("left", checkZ, 0);
