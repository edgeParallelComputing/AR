...
Calibration::Calibration(const int imgWidth, const int imgHeight,
                         ...
                         const float chessboardGridSize,
                         const int MAX_CALIBRATED_IMAGE_COUNT)
    : MAX_CALIBRATED_IMAGE_COUNT(MAX_CALIBRATED_IMAGE_COUNT),
      IMG_WIDTH(imgWidth),
      IMG_HEIGHT(imgHeight),
      IMG_SIZE(cv::Size(imgWidth, imgHeight)),
      CHESSBOARD_WIDTH(chessboardWidth),
      CHESSBOARD_HEIGHT(chessboardHeight),
      CHESSBOARD_GRID_SIZE(chessboardGridSize),
      CHESSBOARD_SIZE(cv::Size(chessboardWidth, chessboardHeight)) {
  ...
  stereoCameraCriteria = TermCriteria(TermCriteria::COUNT + TermCriteria::EPS,
                                      30, 1e-5);
  singleCameraFlag = CV_CALIB_USE_INTRINSIC_GUESS + CV_CALIB_ZERO_TANGENT_DIST
      + CV_CALIB_FIX_K4 + CV_CALIB_FIX_K5;
  stereoCameraFlag = CV_CALIB_USE_INTRINSIC_GUESS + CV_CALIB_ZERO_TANGENT_DIST
      + CV_CALIB_FIX_K4 + CV_CALIB_FIX_K5;
...
bool Calibration::findChessboard(const Mat& img1, const Mat& img2,
  ...
  found1 = findChessboardCorners(img1, CHESSBOARD_SIZE, corners1,
  CV_CALIB_CB_ADAPTIVE_THRESH | CV_CALIB_CB_FILTER_QUADS);
  found2 = findChessboardCorners(img2, CHESSBOARD_SIZE, corners2,
  CV_CALIB_CB_ADAPTIVE_THRESH | CV_CALIB_CB_FILTER_QUADS);
...
void Calibration::finishFindChessboard() {
  for (unsigned int i = 0; i < imgPoints1.size(); i++) {
    ...
    for (unsigned int j = 0; j < imgPoints1[i].size(); j++) {
      v1.push_back(
          Point2f((double) imgPoints1[i][j].x, (double) imgPoints1[i][j].y));
      v2.push_back(
          Point2f((double) imgPoints2[i][j].x, (double) imgPoints2[i][j].y));
    }
...
double Calibration::singleCameraCalibration(const int whichCamera) {
  ...
  if (whichCamera == LEFT) {
    return (leftRet = calibrateCamera(objectPoints, leftPoints, IMG_SIZE,
                                      ...
                                      singleCameraFlag, singleCameracriteria));
  } else {
    return (rightRet = calibrateCamera(objectPoints, rightPoints, IMG_SIZE,
                                       cameraMatrix2, distCoeffs2, R2, T2,
                                       singleCameraFlag, singleCameracriteria));
  }
...
double Calibration::stereoCameraCalibration() {
  return stereoRet = cv::stereoCalibrate(objectPoints, leftPoints, rightPoints,
                                         cameraMatrix1, distCoeffs1,
                                         cameraMatrix2, distCoeffs2, IMG_SIZE,
                                         R, T, E, F, stereoCameraFlag,
                                         stereoCameraCriteria);
}
