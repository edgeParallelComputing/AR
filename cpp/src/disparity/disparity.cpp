...
#ifdef GPU_MODE
void
DisparityMap::calDisparityMapGPU(Mat& leftImg, Mat& rightImg) {
  cuda::GpuMat d_left, d_right;
  //upload mat to gpu
  d_left.upload(left);
  d_right.upload(right);
  d_disp = cuda::GpuMat(left.size(), CV_8U);
  ...
  switch(algorithm) {
    ...
    this->bm = cuda::createStereoBM(112, 9);
...
#ifdef GPU_MODE
void
DisparityMap::cudaDrawColorDisparityMap() {
...
void
DisparityMap::readParam(std::string filename, int algorithm) {
  ifstream readFile;
  ...
  if(readFile.is_open()) {
    ...
    {
      case BM:
      std::string numDisparities, blockSize, minDisparity, textureThreshold, uniquenessRatio, speckleWindowSize, speckleRange, disp12maxDiff;
      while(!readFile.eof()) {
...
void
DisparityMap::printParam(int algorithm) {
  switch(algorithm)
  {
    case BM:
