...
public class NonPipelineBolt extends BaseRichBolt {
	...
	public void execute(Tuple tuple) {
		...
		if(imgId % 1 == 0){
				...
				try{
					DatagramSocket clientSocket = new DatagramSocket();
		      		...
		      		String log_s = nonPipelineBoltsName[0] + " " + String.valueOf(imgId) + " " + String.valueOf(trc[0].elapseTimeDiff()); 
		...
		byte[] disparity = ReconstructionAPI.calculateDisparityMap(uLeft, uRight, imgCols, disparityAlgorithm);

	        ReconstructionAPI.calculateDisparityMap(uLeft, uRight, imgCols, disparityAlgorithm);
	        ReconstructionAPI.calculateDisparityMap(uLeft, uRight, imgCols, disparityAlgorithm);
	        ReconstructionAPI.calculateDisparityMap(uLeft, uRight, imgCols, disparityAlgorithm);
		...
		if(imgId % 1 == 0){
				...
				try{
					DatagramSocket clientSocket = new DatagramSocket();
		      		...
		      		//byte[] sendData = new byte[100];
		      		//sendData = String.valueOf(imgId).getBytes();
		      		String log_s = nonPipelineBoltsName[1] + " " + imgId + " " + trc[1].elapseTimeDiff(); 
		      		byte[] sendData = log_s.getBytes();
		      		DatagramPacket sendPacket = new DatagramPacket(sendData, sendData.length, IPAddress, serverPort);
		      		clientSocket.send(sendPacket);
		      		sendPacket = null;
		...
		if(imgId % 1 == 0){
				...
				try{
					DatagramSocket clientSocket = new DatagramSocket();
		      		...
		      		//byte[] sendData = new byte[100];
		      		//sendData = String.valueOf(imgId).getBytes();
		      		String log_s = nonPipelineBoltsName[2] + " " + imgId + " " + trc[2].elapseTimeDiff(); 
		      		byte[] sendData = log_s.getBytes();
		      		DatagramPacket sendPacket = new DatagramPacket(sendData, sendData.length, IPAddress, serverPort);
		      		clientSocket.send(sendPacket);
		      		sendPacket = null;
		...
		if(imgId % 1 == 0){
			...
			//byte[] sendData = new byte[100];
			//sendData = String.valueOf(imgId).getBytes();
			String log_s = String.valueOf(imgId);
			byte[] sendData = log_s.getBytes();
			DatagramPacket sendPacket = new DatagramPacket(sendData, sendData.length, IPAddress, serverPort);
			clientSocket.send(sendPacket);
			sendPacket = null;
		...
		//this.collector.emit("overallLatencyTuples", new Values(imgId, trc[0].getElapseTime()));
