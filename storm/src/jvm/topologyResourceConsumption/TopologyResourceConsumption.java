...
public class TopologyResourceConsumption {
	...
	private double cpuUtilization() {
		...
		return usedCpuTimeDiff * 100 / (double) (elapseTimeDiff * cores * 100 / 100.00);
	}
	...
	public void reportConsumingResource(int imgId) {
	    //count++;
		/* cpu time */
		log.info("[" + boltName + "] task is [" + imgId + "]"
			 + " timestamp is : [" + getElapseTime() + "]"
						  + " CpuTimeDiff is: [" + usedCpuTimeDiff() + "] ms" 
						  + " ElapseTimeDiff is: [" + elapseTimeDiff() + "] ms"
						  + " CpuUtilization is: [" + cpuUtilization() + "] %"
						  + " MemoryDiff is: [" + memoryDiff() + "] MB"
						  + " InputSize is : [" + inputSize * 100 / 100.00 / 1000000 + "] MB"
						  + " OutputSize is : [" + outputSize * 100 / 100.00 / 1000000 + "] MB"
						  + " DownloadBandwidth is : [ " + downloadBandwidth() + "] MBps"
						  + " UploadBandwidth is : [ " + uploadBandwidth() + "] MBps");
